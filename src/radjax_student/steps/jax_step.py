"""Runtime-hosted execution for the explicit JAX learning capability."""
...
def execute_jax_learning_step(
    *,
    ...
    """Execute one JAX update through the established runtime boundary."""

    if not isinstance(architecture, JaxArchitectureExecution):
        raise TypeError("architecture must expose JaxArchitectureExecution")
    ...
    if runtime_result.status != "pass" or output is None:
        raise ValueError("runtime execution failed for JAX learning step")
