...
def save_learning_checkpoint_v3(
    ...
    *,
    ...
    parameter_descriptor = write_deterministic_npz(
        directory / "parameters.npz", checkpoint.parameters
    )
    carry_descriptor = write_deterministic_npz(
        directory / "architecture_carry.npz", checkpoint.architecture_carry
    )
    optimizer_descriptor = write_deterministic_npz(
        directory / "optimizer_state.npz", checkpoint.optimizer_state.arrays
    )
    ...
    manifest = {
        ...
        "hashes": {name: _digest((directory / name).read_bytes()) for name in files},
    ...
    _write_json(directory / "manifest.json", {**manifest, "integrity": integrity})
